...
for j=plotStepSize:plotStepSize:N
    ...
    for i=1:Chains
        ...
        FJKSErrCh4(counter,i)=max(abs(yFJ-gamcdf(xFJ.^p/p,d/p,1)));
    end
    ...
    FJKSMeanErr4(counter)=mean(J0KSErrCh4(counter,:));

    J0WMeanErr4(counter)=mean(J0WErrCh4(counter,:));
    LFWMeanErr4(counter)=mean(LFWErrCh4(counter,:));
    FJWMeanErr4(counter)=mean(J0WErrCh4(counter,:));
end

%% Sample p-Chi Distribution using HMC and CHMC for d=800
d=800;
[qJ0Ch8,qLFCh8,qFJCh8,qJ0Ch8Time,qLFCh8Time,qFJCh8Time]=ChiSampler(d,p,N,Chains,T,dt);
qJ08=reshape(qJ0Ch8,[],1);
qLF8=reshape(qLFCh8,[],1);
qFJ8=reshape(qFJCh8,[],1);
...
histogram(qFJ4,edges,'Normalization','pdf','FaceColor',colorFJ)
hold on
...
for Ch=1:Chains
    ...
    if max(samples)-min(samples)>10*eps
        Violin({samples'},Ch,'QuartileStyle','shadow','DataStyle', 'none','ShowMedian', true,'ShowMean', true, 'ViolinColor',{colorLF});
    end
    samples = qFJCh4(Ch,(isfinite(qFJCh4(Ch,:))));
    Violin({samples'},10+Ch,'QuartileStyle','shadow','DataStyle', 'none','ShowMedian', true,'ShowMean', true, 'ViolinColor',{colorFJ});
    samples = qJ0Ch4(Ch,(isfinite(qJ0Ch4(Ch,:))));
    Violin({samples'},20+Ch,'QuartileStyle','shadow','DataStyle', 'none','ShowMedian', true,'ShowMean', true, 'ViolinColor',{colorCHMC});
end
set(gca,'XTick',[])
hold off
...
semilogy(1,FJKSMeanErr4(1),'color',colorFJ,'linewidth',2);
semilogy(1,FJWMeanErr4(1),'color',colorFJ,'linewidth',2,'LineStyle','--');

semilogy(1,J0KSMeanErr4(1),'color',colorCHMC,'linewidth',2);
semilogy(1,J0WMeanErr4(1),'color',colorCHMC,'linewidth',2,'LineStyle','--');

for j=1:Chains
    ph = semilogy(qLFCh4Time(j,plotStepSize:plotStepSize:N),LFKSErrCh4(:,j),'color',colorLF,'linewidth',2);
    ph.Color = [colorLF, alphaLevel];
    ph = semilogy(qFJCh4Time(j,plotStepSize:plotStepSize:N),FJKSErrCh4(:,j),'color',colorFJ,'linewidth',2);
    ph.Color = [colorFJ, alphaLevel];
    ph = semilogy(qJ0Ch4Time(j,plotStepSize:plotStepSize:N),J0KSErrCh4(:,j),'color',colorCHMC,'linewidth',2);
    ph.Color = [colorCHMC, alphaLevel];

    ph = semilogy(qLFCh4Time(j,plotStepSize:plotStepSize:N),LFWErrCh4(:,j),'color',colorLF,'linewidth',2,'LineStyle','--');
    ph.Color = [colorLF, alphaLevel];
    ph = semilogy(qFJCh4Time(j,plotStepSize:plotStepSize:N),FJWErrCh4(:,j),'color',colorFJ,'linewidth',2,'LineStyle','--');
    ph.Color = [colorFJ, alphaLevel];
    ph = semilogy(qJ0Ch4Time(j,plotStepSize:plotStepSize:N),J0WErrCh4(:,j),'color',colorCHMC,'linewidth',2,'LineStyle','--');
    ph.Color = [colorCHMC, alphaLevel];
end
semilogy(qLFCh4Time(j,plotStepSize:plotStepSize:N),LFKSMeanErr4,'color',colorLF,'linewidth',3);
semilogy(qLFCh4Time(j,plotStepSize:plotStepSize:N),LFWMeanErr4,'color',colorLF,'linewidth',3,'LineStyle','--');

semilogy(qFJCh4Time(j,plotStepSize:plotStepSize:N),FJKSMeanErr4,'color',colorFJ,'linewidth',3);
semilogy(qFJCh4Time(j,plotStepSize:plotStepSize:N),FJWMeanErr4,'color',colorFJ,'linewidth',3,'LineStyle','--');

semilogy(qJ0Ch4Time(j,plotStepSize:plotStepSize:N),J0KSMeanErr4,'color',colorCHMC,'linewidth',3);
semilogy(qJ0Ch4Time(j,plotStepSize:plotStepSize:N),J0WMeanErr4,'color',colorCHMC,'linewidth',3,'LineStyle','--');
...
In=trapz(CurveX,CurveY);
DX=(max(CurveX)-min(CurveX))/2;
histogram(qFJ8,edges,'Normalization','pdf','FaceColor',colorFJ)
hold on
...
for Ch=1:Chains
    ...
    samples = qFJCh8(Ch,(isfinite(qFJCh8(Ch,:))));
    Violin({samples'},10+Ch,'QuartileStyle','shadow','DataStyle', 'none','ShowMedian', true,'ShowMean', true, 'ViolinColor',{colorFJ});
...
semilogy(1,FJKSMeanErr8(1),'color',colorFJ,'linewidth',2);
semilogy(1,FJWMeanErr8(1),'color',colorFJ,'linewidth',2,'LineStyle','--');
...
for j=1:Chains
    ph = semilogy(qLFCh8Time(j,plotStepSize:plotStepSize:N),LFKSErrCh8(:,j),'color',colorLF,'linewidth',2);
    ph.Color = [colorLF, alphaLevel];
    ph = semilogy(qFJCh8Time(j,plotStepSize:plotStepSize:N),FJKSErrCh8(:,j),'color',colorFJ,'linewidth',2);
    ph.Color = [colorFJ, alphaLevel];
    ph = semilogy(qJ0Ch8Time(j,plotStepSize:plotStepSize:N),J0KSErrCh8(:,j),'color',colorCHMC,'linewidth',2);
    ph.Color = [colorCHMC, alphaLevel];


    ph = semilogy(qLFCh8Time(j,plotStepSize:plotStepSize:N),LFWErrCh8(:,j),'color',colorLF,'linewidth',2,'LineStyle','--');
    ph.Color = [colorLF, alphaLevel];
    ph = semilogy(qFJCh8Time(j,plotStepSize:plotStepSize:N),FJWErrCh8(:,j),'color',colorFJ,'linewidth',2,'LineStyle','--');
    ph.Color = [colorFJ, alphaLevel];
    ph = semilogy(qJ0Ch8Time(j,plotStepSize:plotStepSize:N),J0WErrCh8(:,j),'color',colorCHMC,'linewidth',2,'LineStyle','--');
    ph.Color = [colorCHMC, alphaLevel];


end
semilogy(qLFCh8Time(j,plotStepSize:plotStepSize:N),LFKSMeanErr8,'color',colorLF,'linewidth',3);
semilogy(qLFCh8Time(j,plotStepSize:plotStepSize:N),LFWMeanErr8,'color',colorLF,'linewidth',3,'LineStyle','--');

semilogy(qFJCh8Time(j,plotStepSize:plotStepSize:N),FJKSMeanErr8,'color',colorFJ,'linewidth',3);
semilogy(qFJCh8Time(j,plotStepSize:plotStepSize:N),FJWMeanErr8,'color',colorFJ,'linewidth',3,'LineStyle','--');

semilogy(qJ0Ch8Time(j,plotStepSize:plotStepSize:N),J0KSMeanErr8,'color',colorCHMC,'linewidth',3);
semilogy(qJ0Ch8Time(j,plotStepSize:plotStepSize:N),J0WMeanErr8,'color',colorCHMC,'linewidth',3,'LineStyle','--');
...
histogram(qFJ12,edges,'Normalization','pdf','FaceColor',colorFJ,'FaceAlpha',0.5)
histogram(qJ012,edges,'Normalization','pdf','FaceColor',colorCHMC)
plot(CurveX,CurveY/In,'linewidth',3,'color','r')
hold off
set(gca,'XTick',[])
set(gca,'YTick',[])
legend({'Exact PDF','HMC--LF','CHMC--FullJ','CHMC'},'Interpreter','latex')
xlim([xp-0.275 xp+0.275])
xticks([round(xp-0.25,2) round(xp,2) round(xp+0.25,2)])
xiStarMax = (1200-1)^(1/p);
args = (1200-1)*log(xiStarMax)-abs(xiStarMax).^p/p - (1200/p-1)*log(p)-gammaln(1200/p);
...
for Ch=1:Chains
    ...
    samples = qFJCh12(Ch,(isfinite(qFJCh12(Ch,:))));
    Violin({samples'},10+Ch,'QuartileStyle','shadow','DataStyle', 'none','ShowMedian', true,'ShowMean', true, 'ViolinColor',{colorFJ});
...
semilogy(1,FJKSMeanErr12(1),'color',colorFJ,'linewidth',2);
semilogy(1,FJWMeanErr12(1),'color',colorFJ,'linewidth',2,'LineStyle','--');

semilogy(1,J0KSMeanErr12(1),'color',colorCHMC,'linewidth',2);
semilogy(1,J0WMeanErr12(1),'color',colorCHMC,'linewidth',2,'LineStyle','--');

for j=1:Chains
    ph = semilogy(qLFCh12Time(j,plotStepSize:plotStepSize:N),LFKSErrCh12(:,j),'color',colorLF,'linewidth',2);
    ph.Color = [colorLF, alphaLevel];
    ph = semilogy(qFJCh12Time(j,plotStepSize:plotStepSize:N),FJKSErrCh12(:,j),'color',colorFJ,'linewidth',2);
    ph.Color = [colorFJ, alphaLevel];
    ph = semilogy(qJ0Ch12Time(j,plotStepSize:plotStepSize:N),J0KSErrCh12(:,j),'color',colorCHMC,'linewidth',2);
    ph.Color = [colorCHMC, alphaLevel];


    ph = semilogy(qLFCh12Time(j,plotStepSize:plotStepSize:N),LFWErrCh12(:,j),'color',colorLF,'linewidth',2,'LineStyle','--');
    ph.Color = [colorLF, alphaLevel];
    ph = semilogy(qFJCh12Time(j,plotStepSize:plotStepSize:N),FJWErrCh12(:,j),'color',colorFJ,'linewidth',2,'LineStyle','--');
    ph.Color = [colorFJ, alphaLevel];
    ph = semilogy(qJ0Ch12Time(j,plotStepSize:plotStepSize:N),J0WErrCh12(:,j),'color',colorCHMC,'linewidth',2,'LineStyle','--');
    ph.Color = [colorCHMC, alphaLevel];

end
semilogy(qLFCh12Time(j,plotStepSize:plotStepSize:N),LFKSMeanErr12,'color',colorLF,'linewidth',3);
semilogy(qLFCh12Time(j,plotStepSize:plotStepSize:N),LFWMeanErr12,'color',colorLF,'linewidth',3,'LineStyle','--');

semilogy(qFJCh12Time(j,plotStepSize:plotStepSize:N),FJKSMeanErr12,'color',colorFJ,'linewidth',3);
semilogy(qFJCh12Time(j,plotStepSize:plotStepSize:N),FJWMeanErr12,'color',colorFJ,'linewidth',3,'LineStyle','--');

semilogy(qJ0Ch12Time(j,plotStepSize:plotStepSize:N),J0KSMeanErr12,'color',colorCHMC,'linewidth',3);
semilogy(qJ0Ch12Time(j,plotStepSize:plotStepSize:N),J0WMeanErr12,'color',colorCHMC,'linewidth',3,'LineStyle','--');
...
hold(ax,'off');
leg = legend([labelLFKS, labelFJKS, labelCHMCKS, labelLFW1, labelFJW1, labelCHMCW1 ],'Interpreter','latex','Orientation','horizontal','Location','south');
